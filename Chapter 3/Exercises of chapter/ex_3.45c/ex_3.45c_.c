...
int main (void){
	...
	float e = 1;	//initialize e by 1 to avoid 1+ calculating ---> e = 1 + 1/1! + 1/2!+ .....
	...
	unsigned int power;	//e^power

	printf ("Enter the power of e^(power) (-1 to end): ");
	scanf ("%d", &power);	//avoid garbage
	...
	while (power != -1){
		while (counter <= 10){	//number of e parts till .... + 1/!10
			...
			while (factorialCounter <= counter){	//if factorial counter less than the number of factorial
				factorial = factorial * factorialCounter;	// multiply factorial for the number
				++factorialCounter;	//increment for factorial counter
			}

			e = e + (pow(power, counter) / factorial);	//calculation for the e parts e = 1 + 1/!1 + 1/!2 + ..... + 1/!10 (counter responsible on that) and power increment
			...
			printf ("e^%u = %f\n",power, e);	//print the result of e approximately e = 2.718282
			e = 1;	//reset e to 1
